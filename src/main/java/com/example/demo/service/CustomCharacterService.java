...
public class CustomCharacterService {
	...
	public CustomCharacterService(CustomCharacterDao customCharacterDao) {
	  ...
	  // 캐릭터 저장 
	 public void customCaracterBySave(CustomCharacter character) {
		 customCharacterDao.customCaracterBySave(
	            character.getMemberId(),
	            character.getSkinColor(),
	            character.getHair() != null ? Integer.valueOf(character.getHair()) : null,
	            ...
	            character.getAccessory() != null ? Integer.valueOf(character.getAccessory()) : null
	        );
	    }


	// 캐릭터 업데이트 
	 public void customCaracterByUpdate(CustomCharacter character) {
	        customCharacterDao.customCaracterByUpdate(
	        		 character.getMemberId(),
	                 character.getSkinColor(),
	                 character.getHair() != null ? Integer.valueOf(character.getHair()) : null,
	                 character.getHairColor(),		 
    ...
    public CustomCharacter convertJsonToCharacter(JsonNode avatarInfo, int memberId) {
        ...
        if (parts.has("accessory")) {
            character.setAccessory(parts.path("accessory").path("style").asInt(0));
        }
    ...
    public JsonNode convertCharacterToJson(CustomCharacter character) {
        ...
        // 액세서리
        if (character.getAccessory() != null) {
            ObjectNode accessory = objectMapper.createObjectNode();
            accessory.put("style", character.getAccessory());
            accessory.put("color", (String) null);
            parts.set("accessory", accessory);
        }
